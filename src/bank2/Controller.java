	...
	public void start() {
		...
		while (true) { // Tomcat 의 방식을 모방
			...
			case "2":
				...
				account = new WageAccount(uid, accountType);

				/*
				 * [한빛뱅크] 계좌번호 : 123456 예금종류 : 저축예금 송상훈님 가입일 : 2016년 12월 14일 잔액
				 * : ￦0
				 */
				while (true) {
				...
				break;
			case "3":
				JOptionPane.showMessageDialog(null,	account.deposit((Integer.parseInt(JOptionPane.showInputDialog("입금할 금액을 입력해주세요.")))));
				break;
			case "4":
				JOptionPane.showMessageDialog(null,	account.withdraw((Integer.parseInt(JOptionPane.showInputDialog("출금할 금액을 입력해주세요.")))));
				break;
			case "5":
				break;
			case "6":
